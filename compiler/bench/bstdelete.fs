\ expected: 99999
\ BST delete 10K nodes, count remaining

100000 constant N
10000 constant D
create left N cells allot
create right N cells allot
create key N cells allot
create deleted N allot
variable ncount

: insert ( val -- )
  ncount @ 0= if
    dup key 0 cells + !
    -1 left 0 cells + !
    -1 right 0 cells + !
    0 deleted 0 + c!
    1 ncount !
    drop exit
  then
  0
  begin
    over key over cells + @
    2 pick < if
      dup cells right + @
      dup 0< if
        drop
        ncount @ over cells right + !
        2 pick ncount @ cells key + !
        -1 ncount @ cells left + !
        -1 ncount @ cells right + !
        0 ncount @ deleted + c!
        1 ncount +!
        2drop exit
      then
      nip
    else
      dup cells left + @
      dup 0< if
        drop
        ncount @ over cells left + !
        2 pick ncount @ cells key + !
        -1 ncount @ cells left + !
        -1 ncount @ cells right + !
        0 ncount @ deleted + c!
        1 ncount +!
        2drop exit
      then
      nip
    then
  again ;

: mark-delete ( val -- )
  0
  begin
    dup 0>= while
    over key over cells + @
    2 pick = if
      1 over deleted + c! 2drop exit
    then
    over key over cells + @
    2 pick < if
      cells right + @
    else
      cells left + @
    then
  repeat
  2drop ;

: lcg ( n -- n' ) 1103515245 * 12345 + 2147483647 and ;

: build-tree ( -- )
  0 ncount !
  N 0 do i insert loop ;

: delete-some ( -- )
  12345
  D 0 do
    lcg dup N mod mark-delete
  loop
  drop ;

: count-remaining ( -- n )
  0 N 0 do
    i deleted + c@ 0= if 1+ then
  loop ;

: main build-tree delete-some count-remaining . cr ;
